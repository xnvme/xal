...
main(int argc, char *argv[])
{
	...
	xal->state->magic = 0;
	failures += expect_attach(-EAGAIN, "attach to a region not yet published");

	xal->state->magic = XAL_SHM_MAGIC + 1;
	failures += expect_attach(-EPROTO, "attach to a region with the wrong magic");

	xal->state->magic = XAL_SHM_MAGIC;
